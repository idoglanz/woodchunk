...
def render_assembly_table(assembly: Assembly, catalog: WoodTypeCatalog, index: int, project=None):
    ...
    with st.expander(f"📦 {assembly.name}", expanded=True):    
        ...
        if not edited_df.equals(df):
            ...
            if "project_manager" in st.session_state and project and project.name != "Untitled Project":
        ...
        with col2:
            if st.button("🗑️", key=f"del_{index}", help="Delete assembly"):
                if project and project.assemblies:
                    ...
                    if "project_manager" in st.session_state and project.name != "Untitled Project":
                        st.session_state.project_manager.save_project(project)
                    return True
    return False
...
def handle_table_edit(edited_data: list, assembly: Assembly, catalog: WoodTypeCatalog):
    ...
    for row in edited_data:
        # Skip empty rows
        if pd.isna(row["Wood Type"]) or pd.isna(row["Length (m)"]) or pd.isna(row["Quantity"]):
            continue
            
        # Find the wood type index from the selection
        wood_type_str = row["Wood Type"]
        wood_type_index = next(
            (i for i, wt in enumerate(catalog.get_all_wood_types())
             if f"{format_dimensions(wt.width, wt.height)} - {wt.description}" == wood_type_str),
            row.get("_wood_type_index", 0)
        )
        
        new_pieces.append(AssemblyPiece(
