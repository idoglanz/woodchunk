...
class WoodTypeCatalog:
        ...
        for index, row in edited_rows.items():
            ...
            if 'Available Lengths' in row:
                try:
                    ...
                    available_lengths = [float(x.strip()) for x in lengths_str.split(",") if x.strip()]
                except:
            ...
            updated_wood_type = WoodType(
                ...
                price_per_meter=float(row.get('Price/m', self.wood_types[index].price_per_meter).strip('₪')),
    ...
    def find_wood_types(self, width: Optional[float] = None, height: Optional[float] = None) -> List[WoodType]:
